...
/*--------------------------------------------------------------*/
    VOID
opt_dial( dial_num, ok_num, can_num )
WORD	dial_num, ok_num, can_num ;
{
 	LONG	tree, junk ;
	WORD	ok ;
...
WORD	item ;
{
    switch ( item )
	{
	case GLOBITM0 : opt_dial( GLOBPREF, OKPGOPT, CANPGOPT ) ;
			break ;
	case SVPFITM0 : make_def() ;
			break ;
	case SCRNITEM :
		if ( DEV_TYPE & SCRN_TYPE )
		    opt_dial( SCRNPREF, OKSCRN, CANSCRN ) ;
		break;
	case PAGEITEM :
	...
	case PTRAYITM :
		if ( DEV_TYPE & ( PRNT_TYPE | PLOT_TYPE ) )
		    opt_dial( TRAYPREF, OKPTRAY, CANPTRAY ) ;
		break ;
	case FILERDIR :
		if ( DEV_TYPE & ( PRNT_TYPE | PLOT_TYPE ) )
		    port_dial() ;
	...
	case FILMITEM :
		if ( DEV_TYPE & CMRA_TYPE )
		    opt_dial( FILMPREF, OKFLMOPT, CANFMOPT ) ;
		break;
	} /* switch */
} /* dial_opt */
