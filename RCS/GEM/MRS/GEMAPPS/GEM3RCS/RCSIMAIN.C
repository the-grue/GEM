...
#define vswr_mode( x )		gsx_1code(SET_WRITING_MODE, x)
#define vsl_type(x)		gsx_1code(S_LINE_TYPE, x)
#define vsl_udstyle(x)		gsx_1code(ST_UD_LINE_STYLE)
...
{      
	...
	VOID		      
copy_colplanes( longaddr, color, isdata)
     	LONG	longaddr;
	WORD	color, isdata;
	{
	UWORD	plane;
...
GRECT * area;
{
	...
	for(k=0; k < rows_in_loop; k++)
	{
	    ...
	    {
	    	src_mp = disp_mfdb.mp;
	    	src_mp += (LONG)((disp_mfdb.fww << 1) * 
				(disp_mfdb.fh * j + (rows_fit * i + k)));
	    	dst_mp = fat_mfdb.mp+ (LONG)(fat_mfdb.fwp *
						(rows_in_loop * j + k));
...
LONG pparmbk;
{
    ...
    rc_intersect(&fat_area, &pt.pb_xc);
    if(rc_equal(&fat_area, &pt.pb_xc)){
	gsx_sclip(&fat_area);
	fattify(NULLPTR);
	return(0);
    }
    ...
    fat2sml(&area, xy, 1, 1);	/*map the point to the data image*/
...
{     
	...
	graf_mouse(M_OFF,0x0L);
        gsx_sclip(&fat_area);
	fattify(NULLPTR);	/*fattify the whole scroll area*/
    ...
    WORD	pxy[];
{
    ...
    pxy[4] = 0;
    pxy[5] = 900;
    v_ellarc(pxy[0], pxy[1], pxy[2], pxy[3], pxy[4], pxy[5]);
    ...
    WORD	pxy[];
{
    ...
    pxy[2] = 1;
    pxy[3] = 1;
    v_ellarc(pxy[0], pxy[1], pxy[2], pxy[3], 0, 3600);
    done = FALSE;
    while (!done)
    {
	if ((kstate & 3) || grid)
...
flash_rec()
{
	    ...
	    v_pline(5, pxy);
	    vsl_type(1);
	    vswr_mode(gl_mode);  /*restore the mode*/
	    flash_on ^= 0xffff;
}
...
do_select()
{
	...
	if(inside(mousex, mousey, &flash_area)){ /*in existing selec.area*/
		...
		if (t1pxy[0] != t2pxy[0] || t1pxy[1] != t2pxy[1]){
		    ...
		    fat2sml(t1pxy, smlxy, 1, 1);  /*destination in data area*/
    ...
    fat2sml(&flash_area, &selec_area, 1, 1);
    flash_on = 0x0000;
    selecton = 3;
    flash_rec();
    graf_mouse(M_ON, 0x0L);
...
do_button()
{
    ...
    vsf_color(colr);
    vsf_interior(1); 	/*solid style for filling pattern*/
	...
	case LINEBOX:
 	...
 	case SARCBOX:
	    rub_arc(t1pxy);
	    while(j--){
		    fat2sml(t1pxy, smlxy, 1, k--);
		    if (rcs_hot == SARCBOX)
		        v_ellpie(smlxy[0], smlxy[1], t1pxy[2]>>3, t1pxy[3]>>3,
			 t1pxy[4], t1pxy[5]);
		    else
		        v_ellarc(smlxy[0], smlxy[1], t1pxy[2]>>3, t1pxy[3]>>3,
		         t1pxy[4], t1pxy[5]);
	    }
	    scr2buf();
	    save_fat(FALSE);
	...
	case HRCTGBOX:
	    ...
	    t2pxy[3] = t1pxy[5];
	    while(j--){
		    fat2sml(t2pxy, smlxy, 2, k);
		    vsl_ends(2, 2);		/* set rounded ends	*/
		    if (rcs_hot == SRCTGBOX)  /* && wp->fill_style)   */
			v_bar(smlxy);
		    else{
			 fat2sml(t1pxy, smlxy, 5, k);
			 v_pline(5, smlxy);
		    }
		    k--;
	    }
	...
	case SOVALBOX:
	    ...
	    while(j--){
		    ...
		    vsl_ends(2, 2);		/* set rounded ends	*/
		    if (rcs_hot == SOVALBOX)
		      v_ellipse(smlxy[0], smlxy[1], t1pxy[2]>>3, t1pxy[3]>>3);
		    else
		      v_ellarc(smlxy[0], smlxy[1], t1pxy[2]>>3, t1pxy[3]>>3, 0, 3600);
            }
	    scr2buf();
	    save_fat(FALSE);
	    gsx_sclip(&fat_area);
	...
	case SPLYGBOX:
		...
		rub_line(&SCRATCH[i]);
		v_pline(2, &SCRATCH[i]);
		fat2sml(&SCRATCH[i], &smlxy[i], 2, k); 
		if (bclicks == 1){
again:		    gsx_attr(FALSE, 1, colr);
		    v_pline(2, &smlxy[i]);
		    if(j==2){
			 fat2sml(&SCRATCH[i], &smlxy[i], 2, k-1);
			 j--;
			 goto again;
		    }
		...
		vsl_ends(2, 2);		/* set round ends	*/
		while(j--){
			fat2sml(SCRATCH, smlxy, i, k--); 
			vsf_color(colr); vswr_mode(1);
			v_fillarea(i++, smlxy);
		}
		scr2buf();
		save_fat(FALSE);
