	...
	REG GRECT	*pc;
{
	...
	if ( (wh == NIL) || 
	     ((wh != DESKWH) && !(D.w_win[wh].w_flags & VF_INTREE))
	   )
	  return(TRUE);
	...
	REG GRECT	*pc;
{
	...
	else
	{
	  ...
	  root = ROOT;
						...
						/* account for drop	*/
						/*   shadow		*/
	pt.g_w += 2;				/* BUGFIX in 2.1	*/
	pt.g_h += 2;

	do_walk(DESKWH, tree, root, depth, &pt);
}
	...
	REG WORD	x, y, w, h;
{
	...
	REG WORD	w_up;
	WORD		w_dn, w_slide, sl_amnt, space, min_sld;
...
w_setactive()
{
	...
	if (gl_wtop == NIL)
	{
	  owner = fpdnm(NULLPTR, 1);
	  d.g_x = d.g_y = d.g_w = d.g_h = 0;
	}
	else
	{
	  owner = D.w_win[gl_wtop].w_owner;	  	  
	  w_getsize(WS_WORK, gl_wtop, &d);
	}
	ct_chgown(owner, &d);
}
	WORD
...
w_menufix()
{
	...
	if (gl_wtop != NIL)
	{ 
	  pid = D.w_win[gl_wtop].w_owner->p_pid;
	  w_setmen(pid);
	}
}
#endif
...
*w_topapp()
{
	if (gl_wtop == NIL)
	  ...
	  return(D.w_win[gl_wtop].w_owner);
}
	...
	WORD	oldwh;
{
	...
	if (gl_wtop != oldwh)
	{
	  if (oldwh != NIL)
	  {
	    ppd = D.w_win[oldwh].w_owner;
 
	    if (ppd != NULLPTR)
	    {
	      if (ppd != rlr)
	      {
	        ap_sendmsg(proc_msg, WM_UNTOPPED, ppd, oldwh, 0, 0, 0, 0);
	
	        for (ii=0; ii<10; ii++)
	          dsptch();
	      }	

	      ap_sendmsg(proc_msg, WM_OFFTOP, ppd, oldwh, 0, 0, 0, 0);
	  ...
	  if (gl_wtop != NIL)
	    ap_sendmsg(proc_msg, WM_ONTOP, w_topapp(), gl_wtop, 0, 0, 0, 0);
	}
}
	...
	LONG	array[];
{
	...
	if (!tree)
	{
	  ...
	  while (wh != NIL)
	  {
	    pid = D.w_win[wh].w_owner->p_pid;
	    if (tree = array[pid])
		break;
	    wh = w_prevobj(W_TREE, ROOT, wh);
	  }
	}
	*ppid = pid;
	return(tree);
...
{
	if (gl_mntree)
	  return(gl_mnppd->p_pid);
	else
	  return(-1);
}
	VOID
w_menu_desk(input_pid, dodesk)
	WORD	input_pid, dodesk;
{
	WORD	pid, ldpid;
	LONG	tree;
/*
dbg("menu_desk, input_pid=%x, dodesk=%x\r\n", input_pid, dodesk);	
*/
	gl_newdesk = 0L; 
	ldpid = gl_ldpid;
	pid = input_pid;
	tree = find_best(&pid, menu_tree);

	if (   (!tree)			/* if no windows with menus 	*/
	    && (input_pid == 1) 	/* and in draw_change()	    	*/
	    && (tree = gl_mntree)	/* try to use the current menu  */
	   )
	  pid = gl_mnppd->p_pid;

	if (tree)
	{
	  gl_newdesk = desk_tree[pid];
	  gl_newroot = desk_root[pid];
	}
	else
	{
	  tree = ad_sysmenu;
	  if ((input_pid == 1) && w_topapp())
	    pid = w_topapp()->p_pid;
	  else 
	    pid  = input_pid;
	...
	if ( (  ( (pid != w_mnpid()) || (tree != gl_mntree)
	  ...
	  pr_load(pid);
        ...
        if ((pid != w_mnpid()) || (tree != gl_mntree))
	{
dbg("installing pid %x menu\r\n", pid);
	  mn_bar(tree, TRUE, pid);
	  dodesk = TRUE; /* if different menu then do desktop too */
	}
	...
	PD		*npd;		/* pd of old process 	*/
{
	...
	for(ii=W_TREE[ROOT].ob_head; ii>ROOT; ii=next)
	{
	  ...
	  if  (
		(
		  !sh[npd->p_pid].sh_isgem
		)
		 ||	 
		(
		  (owner != npd) && 
		  (!(sh[npd->p_pid].sh_state & CONTINUOUS)) &&
	          (!(sh[owner->p_pid].sh_state & CONTINUOUS)) &&
		  (!sh[owner->p_pid].sh_isacc)
		)
	      ) 
	  {
	...
	PD		*npd;		/* pd of new process 	*/
{
	...
	tail = W_TREE[ROOT].ob_tail;
 	
	ii   = W_TREE[ROOT].ob_head;
	...
	WORD		w_handle;
{
	...
	D.w_win[w_handle].w_rlist = (ORECT *) 0x0;
	...
	WORD		w_handle;
{
	if (w_handle > 0)
	{
	  if (D.w_win[w_handle].w_flags & VF_INTREE)
	    wm_close(w_handle);
	  w_delete(w_handle);
	}	
}
