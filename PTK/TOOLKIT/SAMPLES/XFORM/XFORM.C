 ...
 *    Update:     Apr. 20, 1989 for ANSI-C Compilers by Robert Schneider
 *
 ************************************************************************/
...
#include "xfmlib.h"           /* everything for the XLIB */
#include "rclib.h"            /* everything for the RCLIB*/
#include "oblib.h"            /* everything for the OBLIB*/
...
GLOBAL OBJECT FAR *gl_menu;         /* menu tree address                */
...
GLOBAL WORD   mousex, mousey;       /* mouse x,y position               */
GLOBAL WORD   bstate, bclicks;      /* button state, & # of clicks      */
GLOBAL WORD   kstate;               /* key state                        */
GLOBAL UWORD  kreturn;              /* keyboard char                    */
...
/*----------------------------------------------------------------------*/
VOID hndl_dial(tree, def, x, y, w, h)
OBJECT FAR *tree;
WORD def;
WORD x, y, w, h;
{
...
/*   string_addr      */
/*------------------------------*/
BYTE *string_addr(which)      /* returns a tedinfo LONG string addr   */
WORD which;
{
   BYTE *where;
...
/*----------------------------------------------------------------------*/
VOID do_about()         /* display xform Info...    */
{
   OBJECT FAR *tree;
   GRECT box;

   ob_xywh(gl_menu, ABOUITEM, &box);
   rsrc_gaddr(R_TREE, ABOUDIAL, &tree);
   hndl_dial(tree, 0, box.g_x, box.g_y, box.g_w, box.g_h);
   ob_undostate(tree, ABOUOK, SELECTED);
}
...
VOID do_view()
{
   ...
   ob_xywh(gl_menu, VIEWITEM, &box);
   rsrc_gaddr(R_TREE, FITDIAL, &tree);
   ob_dostate(tree, cur_fit, SELECTED);
   hndl_dial(tree, 0, box.g_x, box.g_y, box.g_w, box.g_h);
   ob_undostate(tree, FITOK, SELECTED);
   for (cur_fit = FULLITEM; cur_fit <= IGASITEM; cur_fit++)
      if (ob_isstate(tree, cur_fit, SELECTED))
         break;
   set_xform();
...
VOID rdr_mesg()
{
   ...
   appl_write(gl_apid, sizeof(ms_buf), ms_buf);
}
...
WORD zoom;
{
   ...
   wind_calc(1, wtype, gl_xfull, gl_yfull, gl_wfull, gl_hfull, &x, &y, &w, &h);
...
VOID set_xform()
{
   ...
   {   
      ...
      case SVASITEM :         /* page to work area, preserve aspect */
         ...
         x_sxform(0, 0, PG_WIDTH, PG_HEIGHT,work_area.g_x, work_area.g_y, w, h);
         rdr_mesg();
         break;
...
/* inside of the maximum window and pass those values to set_aspect     */
/* (along with the screen attibutes) to calculate new inside window     */
/* values. Then use wind_calc again to get the outside values of the    */
/* window for wind_set.                                                 */
...
WORD xf_init()
{
   ...
   if (!rsrc_load("XFORM.RSC"))
   {
      ...
      form_alert(1,"[3][Fatal Error !|XFORM.RSC|File Not Found !][Abort]");
      return(1);
   }
   ...
   graf_mouse(HOURGLASS, 0x0L);
   wind_set(xf_whndl, WF_NAME, FPOFF(wdw_title), FPSEG(wdw_title), 0, 0);
   do_open(xf_whndl, gl_wfull/2, gl_hfull/2,
                     gl_xfull, gl_yfull, gl_wfull, gl_hfull);
   graf_mouse(ARROW,0x0L);
   get_work();
   set_xform();
